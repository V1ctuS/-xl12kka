...
<div class='pddInner'>
	...
	<?php } if($funct['trnsf3'] == 1) { ?>
	
		<!-- in-game para online -->
		
		<h2><?php echo $LANG[10013]; ?> - In-game &raquo; Online</h2>
		
		<?php echo $LANG[40056]; ?><br />
		
		<br />
		<form method='POST' action='./?module=donate&engine=convert_balance_online' class='usarJquery'>
			
			<label class='formpadrao'>
				<div>
					<div class='desc'><?php echo $LANG[10168]; ?>:</div>
					<div class='camp'><input type='text' name='count' autocomplete='off' /></div>
				</div>
			</label>
			
			<label class='formpadrao'>
				<div>
					<div class='desc'><?php echo $LANG[10056]; ?>:</div>
					<div class='camp'>
						<select name='char'>
							<?php
							$listChars = Donate::listChars($_SESSION['acc']);
