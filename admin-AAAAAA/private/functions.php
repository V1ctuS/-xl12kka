...
function adminLog($text) {
	DB::Executa("INSERT INTO site_log_admin (log_value, log_ip, log_date) VALUES ('".vCode($text)."', '".$_SERVER['REMOTE_ADDR']."', ".($GLOBALS['l2version'] == 1 ? "GETDATE()" : "NOW()").")"); // Admin Log
}
...
function obtainOrderStatusName($status) {
	switch($status) {
		case 1: return 'Pendiente'; break; // Pendente
		case 2: return 'Deleted'; break; // Excluída (não aparece em nenhum local)
		case 3: return 'Pagado'; break; // Pago
		case 4: return 'Entregado'; break; // Entregue
		case 5: return 'Cancelada'; break; // Cancelada
		default: return 'Pendiente'; break; // Pendente
	}
}
