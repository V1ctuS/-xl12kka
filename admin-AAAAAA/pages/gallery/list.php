...
<section class="content">
	...
	<div class='box-footer'>
		<a href='?page=addimg&module=gallery' class='btn btn-primary'>Añadir Imagen</a> &nbsp;
		<a href='?page=addvideo&module=gallery' class='btn btn-primary'>Añadir Vídeo</a>
	</div>
				...
				if(count($consulta) > 0) {
					...
					for($i=0, $c=count($consulta); $i < $c; $i++) {
						...
						<div id='item_".$consulta[$i]['gid']."'>
							...
							<div class='vis'>".($consulta[$i]['vis'] == '1' ? 'Visible' : ($consulta[$i]['vis'] == '2' ? '<span style="color: #4ac600;">Pendente</span>' : '<span style="color: red;">Oculto</span>'))."</div>
							<div>
								<a href='?page=edit&module=gallery&gid=".$consulta[$i]['gid']."' title='Cambiar' class='btn btn-default'><i class='fa fa-edit'></i></a>
								";
								if($consulta[$i]['vis'] == '1') {
									echo "<a href='?engine=change&module=gallery&gid=".$consulta[$i]['gid']."&vis=0' title='Hacer oculto' class='btn btn-default usarJquery'><i class='fa fa-eye-slash'></i></a>";
								} else {
								...
								echo "
								<a href='?page=delete&module=gallery&gid=".$consulta[$i]['gid']."' title='Borrar' class='btn btn-danger'><i class='fa fa-remove'></i></a>
							</div>
