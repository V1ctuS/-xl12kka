...
if(!is_array($img_pt)){
	fim('Es obligatorio para entrar en el banner en portugués! #1');
}

if(empty($img_pt['name'])){
	fim('Es obligatorio para entrar en el banner en portugués! #2');
}
...
if($ext != '.jpg' && $ext != '.jpeg' && $ext != '.png') {
	fim('Seleccione una imagen en un formato válido!<br />Formatos permitidos: JPG, JPEG y PNG');
}
...
if($inserir){
	...
	adminLog("Agregó un banner (ID ".DB::$lastInsertID.")"); // Admin Log
	fim('¡Banner insertado con éxito!', 'OK', './?page=list&module=banners');
